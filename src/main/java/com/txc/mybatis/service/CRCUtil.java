...
public class CRCUtil {
    ...
    public static String getCRC(String data) {
        System.out.println("校验前原始报文："+ data);
        data = data.replace(" ", "");
        int len = data.length();
        if (!(len % 2 == 0)) {
            return "0000";
...
//        return result.substring(2, 4) + result.substring(0, 2);
     ...
     */
    public static String getCRC2(byte[] data) {
        byte[] crc16_h = {
    ...
    public static boolean crcCheck(ByteBuf buf) throws UnsupportedEncodingException {
        String message = CRCUtil.convertByteBufToString(buf);
        System.out.println("原始报文：" + message);
        String replace = message.replace(" ", "");
        String checkCode = replace.substring(replace.length() - 4, replace.length());
        System.out.println("原始报文校验位："+checkCode);
        String crc = getCRC(replace.substring(0, replace.length() - 4));
        System.out.println("校验后报文：" + crc);
        return crc.equals(checkCode) ? true : false;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(byte2hexNew("http://wx.nyjinguan.com/app/download?code=".getBytes(StandardCharsets.UTF_8)));
    }
